    ...
    extends Mage_Catalog_Model_Product
{
    ...
    public function getSpecialPrice()
    {
        ...
        } else if ($childProduct = $this->getFirstSalable()) {
            return $childProduct->getSpecialPrice();
        }

        if (('simple' == $this->getTypeId()) && Mage::getSingleton('customer/session')->getCustomer() && Mage::getSingleton('customer/session')->getCustomer()->getGroupId()) {
            $key = 'special_price';
            $mag_id = Mage::getSingleton('customer/session')->getCustomer()->getGroupId();
            if ($mag_id && in_array($key, array('price', 'special_price'))) {
