...
function autoJump_onKeyUp(fieldName,nextFieldName,maxLength) {
	...
	var nextField = document.forms[0].elements[nextFieldName];
...
function autoFocus_onLoad() {
    if (chargee) {
      ...
      var field_card_num     = document.forms[0].elements['card_num'];
      var field_passwd       = document.forms[0].elements['passwd'];
      var field_reponse_defi = document.forms[0].elements['reponseDefi'];
      var field_Continuer    = document.forms[0].elements['Continuer'];
      var field_num_user     = document.forms[0].elements['code_usager'];
...
function L_menu(cle) {
	...
	if (chargee) {
		...
		if(cle == "enregistrement"){
			
			url = enreg;
			
		}else if(cle == "inscrire"){

			url = inscrip;

		}else if(cle == "inscrireEnLigne"){
			url = inscrireEnLigne;
...
function MM_openBrWindow(theURL,winName,features) {
	var URLTemp = ""; 
	if(theURL.indexOf("?") != -1 ) {
		var firstPart = theURL.substring(0,theURL.indexOf("?")) + "?moveToken=false&";	
		var secondPart = theURL.substring(theURL.indexOf("?")+1,theURL.length);	
		URLTemp = firstPart + secondPart;
	} else {
		URLTemp	= theURL + "?moveToken=false";
	}	
   	newwindow = window.open(URLTemp,winName,features);
