...
class POS_System_Model_Attr extends Mage_Core_Model_Abstract
{
    ...
    public function getItemByMagentoCode($code, $id)
    {
        return $this->getCollection()
            ->addFieldToFilter('code', $code)
    ...
    public function getRexIdByMagentoCode($code, $id)
    {
        return $this->getItemByMagentoCode($code, $id)->getRexId();
    }
    ...
    public function getMagentoIdByRexCode($code, $id)
    {
        ...
        $result = $conn->query("SELECT * FROM $optionTable WHERE option_id = ?", [$return]);
